...
class Cosmos3VFMTransformer(nn.Module):
    ...
    def _prepare_gen_inputs(
        ...
        *,
        ...
        # Timestep embedding (fp32 for precision). For I2V, conditioned frames do
        # not receive diffusion timestep signal.
        ...
        if noisy_frame_mask is not None:
            ...
            token_noisy_mask = (
                ...
                .unsqueeze(-1)
                .expand(-1, -1, hp * wp)
                .reshape(hidden_video.shape[0], -1, 1)
            )
    ...
    def forward_cached_gen(
        ...
        **kwargs,
        ...
        del fps, action_start_frame_offset, action_fps, transfer_share_vision_temporal_positions
        if kwargs:
